# 13_analyze_metrics.py
# Lê um MIDI, calcula métricas (globais e por instrumento) e exporta um CSV único.
...
# ---------------------------
# Métricas por instrumento / globais
# ---------------------------
...
def bpm_stats_robust(src_path: Path, pmid: pm.PrettyMIDI) -> tuple[float, float, float]:
    """
    Lê set_tempo em TODAS as trilhas via mido (merge), calcula segmentos com BPM vigente
    e retorna (min, mediana ponderada por duração, max). Fallbacks: estimate_tempo -> 120.
    """
    try:
        mid = mido.MidiFile(src_path)
        tpb = mid.ticks_per_beat
        tempo_uspb = 500_000  # default 120 bpm
        merged = mido.merge_tracks(mid.tracks)
        ...
        for msg in merged:
            # delta em segundos usando o TEMPO ATUAL
            if msg.time:
                dt = mido.tick2second(msg.time, tpb, tempo_uspb)
                acc_seconds += dt

            if msg.is_meta and msg.type == "set_tempo":
                # fecha segmento até aqui
                if acc_seconds > 1e-9:
                    seg_bpms.append(cur_bpm)
                    seg_durs.append(acc_seconds)
                    acc_seconds = 0.0
                # atualiza tempo
                tempo_uspb = max(1, int(msg.tempo))  # evita 0
                cur_bpm = 60_000_000 / tempo_uspb

        # segmento final até o fim do arquivo
        if acc_seconds > 1e-9:
            seg_bpms.append(cur_bpm)
            seg_durs.append(acc_seconds)

        # se não houver set_tempo, merged ainda terá tempo acumulado; caímos abaixo
        if seg_bpms:
            seg_bpms = np.asarray(seg_bpms, dtype=float)
            seg_durs = np.asarray(seg_durs, dtype=float)
            # saneamento básico
            seg_bpms = np.clip(seg_bpms, 20.0, 300.0)
            w = seg_durs / seg_durs.sum()
            bpm_min = float(seg_bpms.min())
            bpm_max = float(seg_bpms.max())
            # mediana ponderada (aproximação por quantil com pesos)
            order = np.argsort(seg_bpms)
            cw = np.cumsum(w[order])
            bpm_med = float(seg_bpms[order][np.searchsorted(cw, 0.5)])
            return bpm_min, bpm_med, bpm_max
    except Exception:
        pass

    # Fallback 1: pretty_midi.get_tempo_changes()
    try:
        tempi, _ = pmid.get_tempo_changes()
        if tempi is not None and len(tempi) > 0 and np.all(np.isfinite(tempi)):
            vals = np.clip(np.asarray(tempi, dtype=float), 20.0, 300.0)
            return float(vals.min()), float(np.median(vals)), float(vals.max())
    except Exception:
        pass

    # Fallback 2: estimate_tempo
    try:
        est = float(pmid.estimate_tempo())
        if np.isfinite(est) and 20.0 <= est <= 300.0:
            return est, est, est
    except Exception:
        pass
...
def main():
    ...
    ap.add_argument("--source_midi", type=str, required=True, help="Arquivo .mid a analisar")
    ap.add_argument("--out_dir", type=str, required=True, help="Pasta de saída")
    ap.add_argument("--out_name", type=str, default="metrics.csv", help="Nome do CSV de saída")
    args = ap.parse_args()

    src = Path(args.source_midi)
    if not src.exists():
    ...
    g_row = {
        ...
        "is_drum": "",
        **g,
        "melodic_variety": melodic_variety(pmid),
        "temporal_coherence": temporal_coherence(pmid),
        "loopability": loopability_score(pmid)
    }
    rows.append(g_row)
    ...
    for i, inst in enumerate(pmid.instruments):
        ...
        row = {
            ...
            "bpm_min": g["bpm_min"],
            "bpm_median": g["bpm_median"],
            "bpm_max": g["bpm_max"],
            "bpm_source": g["bpm_source"],
            **m,
        }
        rows.append(row)
